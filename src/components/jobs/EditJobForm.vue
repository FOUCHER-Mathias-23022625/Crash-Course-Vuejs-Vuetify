...
const handleSubmit = async () => {
    ...
    try {
        const response = await axios.put(`/api/jobs/${jobId}`, UpdateJobOffer);
        toast.success('Job Updated Successfully');
        router.push(`/jobs/${response.data.id}`);
    } catch (error) {
        toast.error('Error Updating job offer');
...
<form @submit.prevent="handleSubmit">
            <h2 class="text-3xl text-center font-semibold mb-6">Edit a Job</h2>
